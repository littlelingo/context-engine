...
RECENT_UPDATES=$(find .context/ -name "*.md" -newer /tmp/.ce-session-start 2>/dev/null | head -5)
...
if [ -n "$CODE_CHANGES" ] && [ -z "$RECENT_UPDATES" ]; then
    ...
    echo "{\"additionalContext\":\"REMINDER: Code changes detected but no .context/ updates found. Consider capturing learnings before ending: patterns to .context/patterns/, errors to .context/errors/INDEX.md, insights to .context/knowledge/LEARNINGS.md. Run /ce-learn if needed.\"}"
fi
