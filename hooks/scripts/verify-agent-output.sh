...
CHANGES=$(git diff HEAD --name-only 2>/dev/null | wc -l)

# Check if PRP steps were marked complete (for implementer)
PRP_UPDATES=$(git diff HEAD --name-only 2>/dev/null | grep "PRP.md" | head -1)

# If implementer/reviewer agent ran but no files changed, flag it
if echo "$AGENT_NAME" | grep -qiE "implement|refactor"; then
